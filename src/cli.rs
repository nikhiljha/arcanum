...
fn main() {
    let matches = App::new("arcanum-cli")
        ...
        .subcommand(
            SubCommand::with_name("gencrd").about("generate CRDs arcanum needs to function"),
        )
        .subcommand(SubCommand::with_name("genkey").about("generate a keypair for arcanum"))
        .subcommand(
        ...
        println!(
            ...
            base64::encode(
                ecies_ed25519::encrypt(
                    &public,
                    matches.value_of("STRING").unwrap().as_bytes(),
                    &mut csprng
                )
                .unwrap()
            )
